...
## Docker
A Dockerfile has been added to facilitate exporting and running this project in other environments without needing to install Godot locally. Use `docker build -t libsm64-godot .` and `docker run -it libsm64-godot` to export.

## Requirements
The `requirements.txt` file tracks Python dependencies used for Reinforcement Learning training with Godot RL Agents and Stable Baselines 3.
