"""Shared Stable-Baselines3 training entrypoint for all single-agent envs.

Historically `rl_parkour/train.py`, `rl_chase/train.py` and `rl_brawl/train.py`
were byte-identical copies (228 lines each). They are now thin wrappers around
:func:`main` in this module, so fixes only need to be applied once.

Usage (from the repository root):
    python -m rl_common.train_sb3 --env_path export/linux64/libsm64-godot-demo.x86_64 --timesteps 1000000
or via the backwards-compatible wrappers:
    python rl_parkour/train.py --env_path <binary> ...
    python rl_chase/train.py --env_path <binary> ...
    python rl_brawl/train.py --env_path <binary> ...
"""
import argparse
import os
import pathlib
...
def main(argv: Optional[List[str]] = None):
    ...
    if args.inference:
        obs = env.reset()
        for _i in range(args.timesteps):
