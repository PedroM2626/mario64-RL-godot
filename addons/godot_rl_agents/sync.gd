...
func _initialize():
	# Clear previous agent arrays to avoid duplicates on re-initialization
	all_agents.clear()
	agents_training.clear()
	agents_training_policy_names.clear()
	agents_inference.clear()
	agents_heuristic.clear()
...
func _attempt_connect():
	# Try to connect in background without blocking the game
	if agents_training.size() > 0:
		...
		# Start async connection attempts
		_connect_with_retry()

func _connect_with_retry(max_attempts := 60):
	for i in range(max_attempts):
		if connected:
			return
		print("Connection attempt " + str(i + 1) + "/" + str(max_attempts) + "...")
	...
	push_warning(
		"Couldn't connect to Python server after " + str(max_attempts) + " attempts. " +
		"Using human controls instead. " +
		"Start the training server and the scene will auto-connect."
	)

func _initialize_training_agents():
	# Now handled by _attempt_connect() for non-blocking behavior
	pass
...
func connect_to_server():
	...
	var attempts = 0
	while stream.get_status() < 2 and attempts < 10:
		stream.poll()
		attempts += 1
		OS.delay_msec(100)
